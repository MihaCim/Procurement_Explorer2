...
class CompanyData:
    ...
    def to_xml(self) -> str:
        ...
        if self.address:
            ...
            ET.SubElement(address_element, "street").text = self.address.get("street", "")
            ET.SubElement(address_element, "city").text = self.address.get("city", "")
            ET.SubElement(address_element, "country").text = self.address.get("country", "")
        ...
        for person_name, details in self.key_individuals.items():
            person_element = ET.SubElement(key_individuals_element, "person", name=person_name)
        ...
        for relationship, details in self.key_relationships.items():
            rel_element = ET.SubElement(key_relationships_element, "key_relationship", name=relationship)
            for key, value in details.items():
        ...
        for risk_level, details in self.risk_level.items():
            level_element = ET.SubElement(risk_levels_element, "risk_level", name=risk_level)
            for key, value in details.items():
